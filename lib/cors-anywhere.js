...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var requestDataEndPoints = ["/oauth/token"];
    var authHeaderEndPoints = ["/access/getAuthorizationCode"];
    ...
    if(requestDataEndPoints.indexOf(location.pathname) !== -1 || authHeaderEndPoints.indexOf(location.pathname) !== -1) {      
      if(authHeaderEndPoints.indexOf(location.pathname) !== -1) {
        req.headers.Authorization = 'Basic '+ (base64.encode(corsAnywhere.clientID+":"+corsAnywhere.apiSecret));
      }
      if(requestDataEndPoints.indexOf(location.pathname) !== -1) {
        req.url = addParameterToURL("client_id="+corsAnywhere.clientID+"&client_secret="+corsAnywhere.apiSecret, req.url);        
      }
    }
    console.log(req.url)
    var location = parseURL(req.url.slice(1));
    ...
    if (!hasRequiredHeaders(req.headers)) {
      res.writeHead(500, 'Header required', cors_headers);
      res.end();
      return;
    }
